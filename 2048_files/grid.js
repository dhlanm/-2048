...
Grid.prototype.addRandomTile = function () {
	if (this.cellsAvailable()) {
		var value = Math.random() < .25 ? 2 : Math.random() < 1/3 ? -2 : Math.random() < .5 ? -4 : 4;
	...
	traversals.x.forEach(function (x) {
		traversals.y.forEach(function (y) {
			...
			if (tile) {
				...
				if (next && Math.abs(tile.value) === Math.abs(next.value) && !next.mergedFrom) {
					var merged = new Tile(positions.next, tile.value + next.value);
					merged.mergedFrom = [tile, next];

					//if (tile.value + next.value !== 0)
...
Grid.prototype.islands = function () {
	...
	var mark = function (x, y, value) {
		if (x >= 0 && x <= 3 && y >= 0 && y <= 3 &&
			self.cells[x][y] &&
			self.cells[x][y].value == value && !self.cells[x][y].marked) {
...
Grid.prototype.monotonicity2 = function () {
	...
	for (var x = 0; x < 4; x++) {
		...
		while (next < 4) {
			...
			var currentValue = this.cellOccupied({
				x: x,
				y: current
			}) ?
				Math.log(this.cellContent(this.indexes[x][current]).value) / Math.log(2) :
				0;
			var nextValue = this.cellOccupied({
				x: x,
				y: next
			}) ?
